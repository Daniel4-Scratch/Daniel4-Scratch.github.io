...
function prompt() {
  ...
  term.write(`\r\n${colors.green}visitor@${document.location.hostname}${colors.reset}:${colors.blue}${displayPath}${colors.reset}$ `);
}
...
async function init() {
  ...
  await fetch('./assets/logo.txt')
    ...
    .then(text => {
      text.split(/\r?\n/).forEach(line => term.writeln(`${colors.red}${line}${colors.reset}`));
    })
    .catch(error => {
      term.writeln('Error loading file: ' + error);
    });
  ...
  term.writeln(`Type ${colors.red}socials${colors.reset} to see my socials.`);
  term.writeln(`Type ${colors.red}stack${colors.reset} to see my knowledge stack.`);
  prompt();
};
...
init().catch(err => {
  ...
  term.writeln(`${colors.red}Error: ${err.message}${colors.reset}`);
});
